...
Return JSON format: {"vin": "6T1BF9FK9FX540435", "make": "Toyota", "model": "Camry", "year": "2015"}`;

    // تجربة أحدث النماذج المعتمدة بالترتيب
    const modelsToTry = [
      'gemini-2.0-flash',
    ...
    if (!rawText) {
      return res.status(500).json({ error: `فشل الاتصال بالنموذج: ${lastErrorMsg}` });
    }

    // استخراج رقم الشاصي القياسي المكون من 17 حرف ورقم
