...
export const StaticPages: React.FC<StaticPagesProps> = ({ lang, view, onNavigate }) => {
  const isRtl = lang === 'ar';
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '20px', marginTop: '24px' }}>
            <div style={{ padding: '20px', borderRadius: '14px', backgroundColor: '#f8fafc', border: '1px solid #e2e8f0' }}>
              ...
              <p style={{ fontSize: '13.5px', color: '#64748b', margin: 0 }}>المساعدة الفورية والاستفسارات: <strong>97455000000+</strong></p>
            </div>
        ...
        <div>
          ...
          <div style={{ color: '#475569', lineHeight: '1.8', fontSize: '13.5px' }}>
            ...
            <ul>
              ...
              <li><strong>السرية الأمان:</strong> جميع عمليات نقل البيانات والتسجيل مشفرة باستخدام أحدث تقنيات المشفرة الرقمية.</li>
            </ul>
          </div>
        </div>
      )}

      {/* 6️⃣ صفحة الشروط والأحكام (المصممة بحرفية لصالح موجود أوتو وتضمن حق العميل) */}
      {view === 'terms' && (
        <div>
          ...
          <div style={{ color: '#334155', lineHeight: '1.8', fontSize: '13.5px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <div>
              <strong>2. مسؤلية مطابقة القطعة:</strong> يلتزم العميل بتزويد المنصة والكراج برقم الشاسي (VIN) الصحيح للسيارة. وفي حال أرسل العميل بيانات خاطئة، يتحمل العميل رسوم التوصيل عند الاستبدال. بينما يتكفل الكراج بالكامل بتكلفة القطعة والتوصيل إذا أرسل قطعة غير مطابقة للبيانات المزودة.
            </div>

            <div>
              <strong>3. الاسترجاع والضمان:</strong> يُسمح باسترجاع القطعة خلال <strong>3 أيام</strong> من تاريخ الاستلام، بشرط أن تكون بنفس حالتها الأصلية وبغلافها بدون تركيبة أو تلف. لا تشمل سياسة الاسترجاع القطع الكهربائية التي تعرضت للاحتراق أو الفحص التجريبي الخاطئ من قبل الفنيين غير المعتمدين.
            </div>
